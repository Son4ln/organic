<?php
	switch ($action) {
	  ...
	  case 'addCertifToProductAction':
	  	$certif = new Certificates();
	  	foreach ($_POST['prodId'] as $key) {
	  	  if($key != 0){
	  	  	$product = $key;
	  	  	$certificate = $_POST['certif-id'];
	  	  	$certif -> addCertificateToProduct ($product, $certificate);
	  	  }
	  	}

	  	$mes = "Thêm chứng nhận vào sản phẩm thành công";
	    $action = "certificateAndProductList&id=$certificate";
	    $typeOfMes = 'alert-success';
	    redirect($action,$mes,$typeOfMes);
	    break;
	  ...
	  case 'certificateAndProductDel':
	  	...
	  	$mes = "Xóa sản phẩm đi kèm chứng nhận thành công";
	    $actiona = "certificateAndProductList&id=$certifId";
	    $typeOfMes = 'alert-success';
	    redirect($actiona,$mes,$typeOfMes);
	    break;
